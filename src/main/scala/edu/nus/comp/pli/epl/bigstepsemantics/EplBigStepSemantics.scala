...
object EplBigStepSemantics {
  ...
  def eval(ast: Expression): Result = ast match {
    ...
    case Bin(Eq, exprLeft, exprRight) => (eval(exprLeft), eval(exprRight)) match {
      ...
      case (ResultBool(valueLeft), ResultBool(valueRight)) => ResultBool(valueRight == valueLeft)
      case _ => EvaluationError
    }
    ...
    case IfElse(cond, ifbranch, elsebranch) => (eval(cond), eval(ifbranch), eval(elsebranch)) match {
      case (ResultBool(cond), ResultInt(ifbranch), ResultInt(elseBranch)) if (cond == true) => ResultInt(ifbranch)
      case (ResultBool(cond), ResultBool(ifbranch), ResultBool(elseBranch)) if (cond == true) => ResultBool(ifbranch)
      case (ResultBool(cond), ResultInt(ifbranch), ResultInt(elseBranch)) => ResultInt(ifbranch)
      case (ResultBool(cond), ResultBool(ifbranch), ResultBool(elseBranch)) => ResultBool(ifbranch)
      case _ => EvaluationError
    }
